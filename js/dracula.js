...
Raphael.fn.connection = function (obj1, obj2, style) {
    ...
    var edge = {
        ...
        draw: function () {
            ...
            var res =
                dis.length == 0 ? [0, 4] : d[Math.min.apply(Math, dis).toFixed(3)];
            /* bezier path */
            var x1 = p[res[0]].x,
                y1 = p[res[0]].y,
                x4 = p[res[1]].x,
            ...
            ((edge.label &&
                ...
                (edge.label = selfRef
                    .text((x1 + x4) / 2, (y1 + y4) / 2, style.label)
                    .attr({
                        fill: "#000",
...
Graph.Node = function (id, node) {
    ...
    node.hide = function () {
        this.hidden = true;
        this.shape &&
        this.shape.hide(); /* FIXME this is representation specific code and should be elsewhere */
        for (i in this.edges)
...
Graph.Renderer.Raphael = function (element, graph, width, height) {
    ...
    d.onmousemove = function (e) {
        ...
        if (selfRef.isDrag) {
            ...
            selfRef.isDrag.set.translate(
                clientX - Math.round(selfRef.isDrag.dx),
                clientY - Math.round(selfRef.isDrag.dy)
            );
    ...
    d.onmouseup = function () {
        selfRef.isDrag && selfRef.isDrag.set.animate({ "fill-opacity": 0.6 }, 500);
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function (node) {
        ...
        if (node.shape) {
            ...
            node.shape.translate(
                Math.round(point[0] - opoint.x),
                Math.round(point[1] - opoint.y)
            );
            this.r.safari();
            return node;
        } /* else, draw new nodes */
    ...
    drawEdge: function (edge) {
        ...
        if (edge.source.hidden || edge.target.hidden) {
            edge.connection &&
            edge.connection.fg.hide() | edge.connection.bg &&
